...
\item {\bf Getting familiar with the {\em Timescales} printout.}  
	...
	\item Our Universe is about 13.8 Billion years old (that is, $1.38 \times 10^{10}$ years).  The calendars/clocks to the left of each graph are meant to express the time period of that row's axis as a fraction of a year, where one year represents the entire age of the Universe.  
		%
		\begin{itemize}
		\item The first row's calender is shaded in to give this representation.  Looking the timespan of this first row, calculate/explain why that portion of the calendar is shaded.
		\item Calculate the fraction of the year represented by the timespans of rows 2--4 and then shade the calendar (or ``remaining time on the clock'') in each row.  (You should first figure out why I've only shown, e.g., December for the second row, and what the clocks are meant to represent.)
		\end{itemize}
		%
	\item Consider the full range of time of the second row's axis.  Indicate the time range of the second row {\em on the graph of the first row} using a vertical line going through the first row's graph.  Then indicate the third row's timescale on the second row's graph; and the fourth row's timescale on the third row's graph.
	\end{enumerate}
...
\item {\bf Plotting events on the ``Timescales'' printout} Using the provided list of events in the history of Earth (which includes geological, biological/life, human history, and ``personal history'' types of events), plot some events on the five timescale graphs. These are some rules/guidelines that you must follow:
	%
	\begin{itemize} 
	\item You are welcome use any of the events --- and add your own events (from your History and Earth Science courses, and your life) --- but {\bf events on the list marked with a star ($\bigstar$)} should definitely be plotted.
...
\item {\bf Atmospheric gases over time} A graph that estimates atmospheric carbon dioxide over time is provided.  
	...
	\item On the third row of the {\em Timescales} graph, put tick marks on the far right axis to create a ${\rm CO}_2$ scale that will span the range of $\text{CO}_2$ concentration values in that time period.
	\item On the third row of the {\em Timescales} graph, indicate with stars the values of ${\rm CO}_2$ at the time of at least three ``ice age'' and three ``interglacial'' periods.
	\item Look online for information about estimated atmospheric ${\rm CO}_2$ concentrations from the time period in row two of {\em Timescales}.  Make an axis at right and plot a few points.
	\item Do you notice any association/correlation between temperature and $\text{CO}_2$ values?
	\end{enumerate}
...
\item Data for the temperature/temperature-proxy plots was collected by Glen Fergus to create the very nice wikipedia graph \href{https://commons.wikimedia.org/wiki/File:All_palaeotemps.svg}{``All Paleotemps''}.  As described on that page, the original published data sets for each row are:
	...
	\item[{\bf  2:}]  Zachos et al (Nature 2008) as interpreted by Hansen et al (Phil Trans RSA 2013); (with Veizer data).
	...
	\end{itemize}
	%
The temperature proxy itself is generally a ratio of isotopes of a particular element  (often oxygen) found either in the bubbles of ice cores or incorporated in the fossilized shells of marine life.  The incorporation of rare isotopes depends on the temperature of the atomosphere/ocean.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{center}
{\bf Atmospheric $\text{CO}_2$ Data (from recent measurements and ice core bubbles)}\\[12pt]
...
$\bigstar$ Formation of our Solar System\\
$\bigstar$ Formation of our Earth\\
$\bigstar$ Formation of the Moon\\
``End of late heavy bombardment''\\
Departure of today's light from Alpha Centauri\\
Formation of oceans\\
Supercontinents (Rodinia, Pannotia, $\bigstar$ Pangea)\\
Last glacial maximum (ice age)\\
Last ``interglacial period''\\
First ice age\\
First ``interglacial''\\
$\bigstar$ Formation of the Great Lakes\\
...
\underline{Ancient Societies}: Greek, Egypt, Sumer, Shang\ldots? ($\bigstar$ one of them)\\
Roman Empire\\
