...
\newpage
	...
	Now consider the again the two pictures of charge pairs  (reproduced below).  In each picture, draw: (i) the field of the left charge ($q_1$) everywhere in the picture; (ii)  the electric field vector, $\vec{E}_1$, at the position  of the second charge; (iii) The force vector on the second charge, $\vec{F}_{E_1 \text{ on } q_2}$.
	%
	\begin{centerinpage}
...
\newpage
	...
	\noindent \underline{Add the electric field to the picture above}: (iv) In between the $x$ axis and the sine curve, draw electric field vectors with their tails on the axis: they will point in the positive $y$ direction when the curve is positive, in the negative direction when the curve is negative, and the vector will go to zero twice per wavelength. This is a representation of a {\em plane wave} of light, so the electric field vector you see at one position on the $x$ axis actually represents the electric field at all locations on an (infinite) 2D plane perpendicular to the propagation direction, $x$. \\[-6pt]
	
	\noindent \underline{In the space below}: (v) Draw an electric field that fills the space, mimicking your plane wave above. Use many $\vec{E}$ vectors (not ``field lines'' like prior questions).  Along any vertical line you should have multiple $\vec{E}$ vectors of the same length. (vi) Draw some electric dipoles,  sitting in this field and indicate the forces on their pairs of charges.

	\vspace{4cm}

	\noindent \underline{Look up the size of a few molecules listed on the first page.} We found earlier that the wavelength of infrared light is about one to 10 micrometers ($\unit{1}{\micro\meter}  = \unit{10^{-6}}{\meter}$).  How do these two sizes compare?  Would there be a lot or a little variation of the $\vec{E}$ field strength across a molecule at any moment in time as an EM wave passes by?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item {\bf Chemistry: Atoms, Molecules, Geometry, Dipoles}
	%
	\begin{enumerate}
	\item .
