...
tempbounds_for_row = [ [-7, 17], [-7, 18], [-7, 6], [-8, 2.5], [-7, 16] ] 
...
for f in data_for_row[plotrow]:
    ...
    if f == "best":
        ax.scatter(arr[:,0], arr[:,1], c = file_color[f], alpha=plot_alpha, label="BerkelyEarth temperature")
    else:
        ax.plot(arr[:,0], arr[:,1], c = file_color[f], alpha=plot_alpha, label=f)
#--- Set axis bounds and decorate
# legend
ax.legend()
# axes labels
ax.set_xlabel("Time since present (years)")
...
if plotrow < 4:
    ax.set_xlim(left_time_for_row[plotrow], -200)
else:
