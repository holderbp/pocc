...
\noindent {\bf Analogy to water and water flow}
	%
	\begin{itemize}
	
	\item Basic idea is that liquid water (in a room temperature situation, ignoring evaporation) is conserved.  It flows from one object to another, but the net flow into any object must equal the change in the amount of water that object contains.
	
	
	\item {\bf Conduction} (transfer of energy from a hot object to a cold object in contact): When two wet objects are in contact (e.g., two sponges, one wet one mostly dry), water will flow from the wetter object to the dryer object (and never in reverse).  The flow stops when the wetness of the two objects is equal.  Wetness is the analog of temperature.
	
	\item {\bf Radiation} (transfer of energy from one object to another by light/photons):  Spray bottle sends small particles of water (if the analogy held, then the color of water and the amount per second out of the spray bottle would depend on the temperature (wetness) of source).
