...
\underline{Consider an example system}: cold milk poured into a mug of hot coffee. We will ignore the energy loss out of the system as a whole during the time of mixing, so the above equation holds.  Sketch an energy flow diagram for these two objects during the time they are mixing (not yet at equilibrium). Then write down three FLTD equations, like those above: one for the combined system and one each for the objects in your diagram.  Instead of ``1'' and ``2'' labels, use ``C'' and ``M''.  
...
Before looking at the heating of an aluminum block under the radiation of the sun (or incandescent bulb), we will first determine the specific heat capacity of the material itself.  Our experiment will be a very simple calorimetry example: prepare a small aluminum cylinder at a known temperature (letting it equilibrate with ice water) and then drop it into a vat of water while monitoring the temperature of the water.  The drop in water temperature at that instant can be associated to a heat transfer from water to aluminum, which causes an internal energy decrease/increase, respectively. Knowing the specific heat water (and the masses of both the water and aluminum), we can determine the specific heat of aluminum.  
...
\noindent {\bf Procedure}: Using your own paper to write down observations, keep a data table, and make calculations from equations, take the following basic steps.
	...
	\item Get hot water from the faucet ($~50\degree$C) and fill the small calorimetry cup with about \unit{2}{\centi\meter} (we will measure this amount after the experiment is over).
	\item Place the cup inside the calorimetry can and observe the temperature through the hole in the cork.
	\item Start the stopwatch and take temperature measurements for at least 5 minutes, at approximately 30 second intervals\footnote{If you would like, you could use the go-pro-like camera to record the stopwatch and thermometer --- probably best to record with the ``time-lapse'' feature --- and then get (temperature, time) data points by watching the video afterwards.  Think carefully about how to do this ahead of time.}. 
	\item While you are still getting temperature measurements on the water,  use a second thermometer, to check that the ice water and cylinder mixture is at the right temperature.
	\item Remove the cylinder using the tongs and drop it into the warm water.
	\item Record the time that the cylinder was added.
	\item Continue to measure the temperature for another 5 minutes, at 30 second intervals.
	\item Create a table of your temperature vs.\ time data.  It is probably easiest to simply record the time on the stopwatch (4m50s) and then later you can convert  that (in an additional column) to the number of seconds (290~s).
	...
	\item At the time the cylinder was inserted, estimate the vertical distance (temperature distance) between these two lines.  Also record the ``final'' temperature (the bottom of this range.
	...
	\item Take your small calorimetry cup to the sink, along with your 100~mL graduated cylinder.  Pour the water into the graduated cylinder to record the volume of water in the can.  (Think ahead of time about how to do this if your water turns out to be more than 100~mL in volume!)
	\item Draw an energy flow diagram that will captures the heat flow and energy changes during the time immediately after placing the aluminum cylinder in hot water, but before it equilibrated.  
	\item Then write down equations to arrive at a $Q_1 + Q_2 = 0$ equation, like you did in the introduction. Use this final equation to calculate the specific heat capacity of aluminum. Recall that the specific heat capacity of water is $c_W = \unit{4.2}{\joule/\gram/\;\degree C}$.
	\end{itemize}
...
\noindent {\bf Procedure:} Using your own paper to write down observations, keep a data table, and make calculations from equations, take the following basic steps.
	%
	\begin{itemize}
	\item Place your aluminum block into the insulating styrofoam.  If there is no hole through the styrofoam for the thermometer, measure the position of the hole in the block and carefully figure out where to pierce the styrofoam such that the thermometer will hit the hole.
	\item Prop up the styrofoam at an angle, using its kickstand, the no-slip pad and a large (e.g., 1~kg) mass.
	\item Arrange the desklamp so that it is pointing at the block and quite close ($\sim \unit{5}{\centi\meter}$). Record the distance.
	\item Start your stopwatch.
	\item Then turn on your desk lamp.
	\item Record (time, Temperature) values in a table, every 30 seconds (or even every minute should be fine) for about 20 minutes.
	\item Make a graph of your data, Temperature vs.\ time.
	\item Using a ruler, draw a straight line of best fit.  Then, using two widely-separated {\em points on the line} calculate the slope of that line.
	...
	\item The power increase you just calcuated can be translated into a flux, by dividing by the area of the aluminum block. Write out an equation for this, and then calculate a value.
